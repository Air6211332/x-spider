...
export const Likes: React.FC = () => {
  ...
  const ready = useLikesStore((s) => s.ready);
  const items = useLikesStore((s) => s.items);
  const totalPosts = useLikesStore((s) => s.totalPosts);
  const stats = useLikesStore((s) => s.stats);
  const lastSyncAt = useLikesStore((s) => s.lastSyncAt);
  const lastFullSyncAt = useLikesStore((s) => s.lastFullSyncAt);
  const syncing = useLikesStore((s) => s.syncing);
  const rechecking = useLikesStore((s) => s.rechecking);
  const recheckProgress = useLikesStore((s) => s.recheckProgress);
  const downloading = useLikesStore((s) => s.downloading);
  const listLoading = useLikesStore((s) => s.listLoading);
  const listHasMore = useLikesStore((s) => s.listHasMore);
  const filter = useLikesStore((s) => s.filter);

  const setFilter = useLikesStore((s) => s.setFilter);
  const ensureAccount = useLikesStore((s) => s.ensureAccount);
  const reloadFromDb = useLikesStore((s) => s.reloadFromDb);
  const loadMoreFromDb = useLikesStore((s) => s.loadMoreFromDb);
  const syncFull = useLikesStore((s) => s.syncFull);
  const syncIncremental = useLikesStore((s) => s.syncIncremental);
  const recheckLocalFiles = useLikesStore((s) => s.recheckLocalFiles);
  const downloadMissing = useLikesStore((s) => s.downloadMissing);
  ...
  useEffect(() => {
    ...
    ensureAccount()
      ...
      .catch((err: any) => {
        const msg =
          typeof err === 'string' ? err : err?.message || '加载喜欢库失败';
        message.error(msg);
      });
  }, [cookieString, ready]);
  ...
  return (
    <div className="flex flex-col h-screen">
      <div className="shrink-0">
        ...
        <div className="px-4 pb-2 space-y-2">
          <p className="m-0 text-sm text-ant-color-text-secondary">
            登录账号喜欢落库同步；按作者目录下载。改模板后请重新校验文件。
          </p>
          <Space wrap>
            <Button
              type="primary"
      ...
      >
        {!cookieString ? (
          <p className="text-ant-color-text-secondary">请先登录后再查看喜欢</p>
        ) : (
          <MediaSourceDbGrid
            items={items}
