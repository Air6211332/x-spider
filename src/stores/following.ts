...
import { getFollowing, getLatestTweetAt, getSelfUserId } from '../twitter/api';
...
/** 关注列表翻页间隔 */
const FOLLOWING_PAGE_DELAY_MS = 800;
  ...
  syncFull: async () => {
    ...
    try {
      ...
      while (cursor !== null) {
        ...
        // 结束条件：无 cursor / cursor 未变 / 连续空页（API 常在末尾仍返回 Bottom）
        if (!page.cursor) {
          break;
        }
        ...
        if (page.entries.length === 0 || newCount === 0) {
          emptyStreak += 1;
          if (emptyStreak >= 2) {
            log().info('Full sync stop: empty/duplicate pages', emptyStreak);
            break;
          }
        } else {
          emptyStreak = 0;
        ...
        cursor = page.cursor;
        await delay(FOLLOWING_PAGE_DELAY_MS);
      }
